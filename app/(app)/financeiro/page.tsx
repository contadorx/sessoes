    ...
    <div className="mx-auto max-w-3xl">
      <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1">
        ...
        <nav className="ml-auto flex items-center gap-3 text-[12.5px]">
          <Link href={`/financeiro?mes=${anterior}`} className="text-tinta2 hover:text-vaga">
            ← {nomeDoMes(anterior).split(" de ")[0]}
          </Link>
          {mes !== atual && (
            <Link href="/financeiro" className="text-tinta3 hover:text-vaga">
              hoje
            </Link>
          )}
          {seguinte <= atual && (
            <Link href={`/financeiro?mes=${seguinte}`} className="text-tinta2 hover:text-vaga">
      ...
      <section className="mt-10 border-t border-linha pt-6">
        ...
        <ul className="mt-3 max-w-2xl space-y-2 text-[13px] leading-relaxed text-tinta2">
          ...
          <li>
            ...
            <Link href="/documentos" className="underline underline-offset-2 hover:text-vaga">
              Documentos
            </Link>{" "}
