-- =====================================================================
-- RENOMEADA EM 03/09 · era `0052_as_leituras_da_revisao_5.sql`
-- =====================================================================
--
-- O nome antigo punha este arquivo **antes** da `0052a`, e o `0052a` existe
-- para derrubar exatamente as views que este arquivo recria com outra forma.
-- Em ordem lexicográfica o `_` (0x5F) vem antes do `a` (0x61):
--
--     0052_a_regua_da_assinatura_e_o_churn_com_causa.sql
--     0052_as_leituras_da_revisao_5.sql       ← quebrava aqui
--     0052a_drop_das_views_que_mudam_de_forma.sql
--
-- e uma base nova morria em `ERROR: cannot change name of view column "n" to
-- "n_total"`, porque `create or replace view` não muda o nome de coluna de uma
-- view que já existe.
--
-- **No banco de produção a ordem sempre foi a certa** — o `schema_migrations`
-- registra a `0052a` às 10:12:48 e esta às 10:13:26 do dia 02/09. Quem estava
-- errado era só o nome do arquivo, e com ele a promessa da lei 5 de que a
-- pasta reconstrói o banco. Sem isto, nenhum dos consertos das outras
-- migrações chega a um restore, a um ambiente novo ou a um segundo par de mãos.
--
-- O registro no Supabase foi renomeado junto, mantendo o `version` — se o nome
-- ficasse antigo, um `supabase db push` veria este arquivo como pendente.
-- =====================================================================
